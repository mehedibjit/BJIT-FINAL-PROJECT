...
from ai.faiss_database.faiss_index import image_index, text_index, image_ids, text_ids, image_descriptions

def multimodal_search(image_query, text_query, alpha=0.5):
    """
...
def search_using_text(query, faiss_index, model, processor, image_ids, threshold=0.85, top_k=10):
    ...
    for idx in range(len(I[0])):
        ...
        # Fix: Calculate the mean cosine similarity across all dimensions
        similarity_score = cosine_similarity(query_embedding, torch.tensor(image_embedding).unsqueeze(0)).mean().item()
        ...
        if similarity_percentage >= threshold * 100:  # Threshold converted to percentage
            similar_images.append((image_path, similarity_percentage)) # Append image path instead of index

    # Sort results by similarity (descending order)
    similar_images.sort(key=lambda x: x[1], reverse=True)
    return similar_images
print("search image with text query")